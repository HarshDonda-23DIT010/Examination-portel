...
const AddSubject = () => {
   ...
   // Filter subjects by user's department
   subjects = subjects.filter(subject => {
      const userDept = user?.department;
      if (userDept === 'DIT') return subject.dep_IT;
      if (userDept === 'DCE') return subject.dep_CE;
      if (userDept === 'DCS') return subject.dep_CSE;
      return false;
   });

   // Filter faculties for the same department - REMOVED FILTER TO SHOW ALL FACULTIES
   let faculties = facultiesData?.data || [];
   // faculties = faculties.filter(faculty => faculty.department === user?.department);
   ...
   const handleUpdateSubmit = async (e) => {
      ...
      try {
         await updateSubject(updateFormData).unwrap();
   ...
   const handleViewSubject = (subject) => {
      navigate(`/hod/subjects/${subject.id}`, { 
         state: { 
            subject: subject,
            yearId: selectedYearObject?.id,
            semester: selectedSemester 
   ...
   return (
      <div className="min-h-screen bg-gray-50 p-4 lg:p-6">
            ...
            <div className="flex justify-between items-center mb-6">
               <div>
                  ...
                  <p className="text-gray-600 mt-1">
                     Manage subjects for {user?.department} department -
                        ...
                        <div className="space-y-4">
                           ...
                           <div>
                              ...
                              <div className="space-y-2">
                                 <input
                                    type="text"
                                    placeholder="Search coordinator by name, ID, or department..."
                                 ...
                                 >
                                    ...
                                    {filteredFaculties.map(faculty => (
                                       <option key={faculty.id} value={faculty.id}>
                                          {faculty.name} ({faculty.userId}) - {faculty.department}
                                       </option>
                                    ))}
                                 </select>
                              </div>
         ...
         {isUpdateDialogOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center">
               {/* Similar structure as add dialog but with update form */}
               <div
                  className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm"
               ...
               <div className="relative bg-white rounded-lg shadow-xl max-w-6xl w-full mx-4 max-h-[90vh] overflow-y-auto z-10">
                  ...
                  <form onSubmit={handleUpdateSubmit} className="p-6 space-y-6">
                     {/* Similar form structure but with updateFormData */}
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {/* Left Column */}
                        <div className="space-y-4">
                           <div>
                              <label htmlFor="updateCode" className="block text-sm font-medium text-gray-700 mb-2">
                                 Subject Code
                              </label>
                              <input
                                 id="updateCode"
                           ...
                           <div>
                              <label htmlFor="updateCoordinatorId" className="block text-sm font-medium text-gray-700 mb-2">
                                 Subject Coordinator
                              </label>
                              <div className="space-y-2">
                                 <input
                                    ...
                                    onChange={(e) => setUpdateCoordinatorSearch(e.target.value)}
                                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                 />
                                 <select
                                    id="updateCoordinatorId"
                                    name="coordinatorId"
                                    value={updateFormData.coordinatorId}
                                    onChange={handleUpdateInputChange}
                        ...
                        {/* Middle & Right columns similar to add form but using updateFormData */}
                        <div className="space-y-4">
                           <h3 className="text-lg font-medium text-gray-900 border-b pb-2">Hours & Credits</h3>

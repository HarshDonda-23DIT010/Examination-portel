...
const ExamAnalysis = () => {
   ...
   const generatePDFReport = async () => {
      ...
      try {
         ...
         doc.text('Admin - Comprehensive Exam Analysis Report', 20, 25);
         ...
         const examDetails = [
            ...
            ['Subject', examData?.subject ? `${examData.subject.name} (${examData.subject.code})` : 'N/A'],
            ['Date', examData?.date ? new Date(examData.date).toLocaleDateString() : 'N/A'],
            ['Total Marks', examData?.totalMarks?.toString() || 'N/A'],
            ['Effective Marks', examData?.effectiveMarks?.toString() || 'N/A'],
            ['Department', examData?.department || 'N/A'],
            ['Division', examData?.division || 'N/A'],
            ['Batch', examData?.batch === 'NONE' ? 'All Batches' : `Batch ${examData?.batch || 'N/A'}`],
            ['Status', examData?.status || 'N/A']
         ];
         ...
         const statisticsData = [
            ...
            ['Average Marks', `${stats?.averageMarks || 0}/${examData?.totalMarks || 0}`],
            ['Average Effective Marks', `${stats?.averageEffectiveMarks || 0}/${examData?.effectiveMarks || 0}`],
            ['Highest Marks', `${stats?.highestMarks || 0}/${examData?.totalMarks || 0}`],
            ['Lowest Marks', `${stats?.lowestMarks || 0}/${examData?.totalMarks || 0}`],
            ['Median Marks', `${stats?.medianMarks || 0}/${examData?.totalMarks || 0}`],
         ...
         // Add charts and complete the PDF similar to the original...
         // (Continuing with the same PDF generation logic as the original file)

         // Save the PDF
         const fileName = `Admin_${(examData?.name || 'Exam_Analysis').replace(/[^a-zA-Z0-9]/g, '_')}_Complete_Analysis_Report.pdf`;
         doc.save(fileName);

         toast.success('Admin comprehensive PDF report generated successfully!');
   ...
   return (
      <div className="p-6 max-w-7xl mx-auto">
         {/* Breadcrumb Navigation - Admin specific */}
